...
requirePattern(/function startLootAutoSelect\(\)[\s\S]*let best=getRecommendedLootChoice\(\);[\s\S]*if\(!best\|\|best\.comparison\.score<=0\)return;/, 'Auto-select must not start for zero or negative recommendation gains');
...
requirePattern(/function autoEquipRecommendedLoot\(\)[\s\S]*if\(!best\|\|best\.comparison\.score<=0\)return;/, 'Auto-select settlement must preserve manual choice for zero or negative gains');
