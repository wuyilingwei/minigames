...
const model = html.match(/const equipmentSlots=\[[\s\S]*?function initSlots\(\)\{[\s\S]*?\n\}/)?.[0];
const actionModel = html.match(/function buildEquipAction\(item,slotIndex,slots=player\.slots\)\{[\s\S]*?\n\}/)?.[0];
...
const difficultyApply = html.match(/function applyDifficultySelection\(\)\{[\s\S]*?\n\}/)?.[0] || '';
...
for (const [level, traits] of [[0, 0], [3, 1], [6, 2], [9, 3]]) {
  ...
  if (!html.includes(expected)) throw new Error(`Difficulty ${level} equipment trait tier is missing.`);
}
for (const text of ['id:"armorBreak"', 'trait.id==="armorBreak"', 'function buildEquipAction', 'function normalizeEquipmentSlots', 'save.slot5Unlocked||save.useBlood>=6?5:4']) {
  if (!html.includes(text)) throw new Error(`Missing 8.0 equipment contract: ${text}`);
}
