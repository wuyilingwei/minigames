...
| 008 | [Content] 外显作品署名 | 更正本站作者姓名并将十光年完整署名展示在门户卡片 | 让门户与游戏内的创作信息一致且可在进入前读取 | 🔄 进行中 |
| 009 | [UX] 设置内退出与传说菜单排序 | 将退出入口移入设置，并调整传说主菜单的启动顺序 | 让退出入口符合游戏交互语境，优先突出继续与新游戏 | 🔄 进行中 |
| 010 | [Docs] 保留原项目说明 | 将两个原项目的 README 原样放入对应游戏目录 | 随发布物保留运行方式、数据来源、版本和致谢信息 | 🔄 进行中 |
| 011 | [UX] 精简迷你游戏首页 | 将首页更名为迷你游戏并仅保留游戏列表 | 消除收录和静态发布等无关叙述，突出可游玩的内容 | 🔄 进行中 |
