...
- 关键修正：槽位改为 `getEquipmentSlots()` 动态连续列表，低难购买槽为可见第 5 槽，高难附加+购买分别为第 5/6 槽。
- 历史 `slot5Unlocked` 一律迁移为购买槽，并设置 `legacyAddonSlotUnlocked` 保留旧购买者已有普通附加装备。
- Boss 快速模拟中的天使增伤、隐士溢出闪避增伤已接入职业护符倍率。
